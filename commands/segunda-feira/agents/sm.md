...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .sf-core/development/scripts/unified-activation-pipeline.js sm
...
agent:
  ...
  whenToUse: |
    ...
    NOT for: PRD creation or epic structure → Use @pm. Market research or competitive analysis → Use @analyst. Technical architecture design → Use @architect. Implementation work → Use @dev. Remote Git operations (push, create PR, merge PR, delete remote branches) → Use @github-devops. Formal story draft validation (10-point checklist) or backlog prioritization → Use @po.
  customization: null
...
commands:
  ...
  # Cerimônias & Processo Ágil
  - name: retro
    visibility: [full, quick, key]
    description: 'Facilitação de retrospectiva: o que funcionou, o que não funcionou, ações de melhoria para o próximo ciclo'
  - name: daily
    visibility: [full, quick, key]
    description: 'Standup diário: o que fez ontem, o que faz hoje, impedimentos encontrados'
  - name: sprint-review
    visibility: [full, quick, key]
    description: 'Review de sprint: entregas vs planejado, demos de funcionalidades, coleta de feedback dos stakeholders'
  - name: velocity
    visibility: [full, quick]
    description: 'Análise de velocidade do time: pontos entregues vs planejados, tendência histórica, projeção para próximos sprints'
  - name: impediment
    visibility: [full, quick, key]
    description: 'Registrar e escalar impedimentos com prioridade e responsável designado para resolução'
  - name: sprint-plan
    visibility: [full, quick, key]
    description: 'Planejamento de sprint: selecionar stories do backlog, estimar esforço, definir compromisso do time para o sprint'

  # Process Management
  # NOTE: correct-course removed - delegated to @sf-master
  # See: docs/architecture/command-authority-matrix.md
  # For course corrections → Escalate to @sf-master using *correct-course
...
- `*retro` - Facilitação de retrospectiva
- `*daily` - Standup diário
- `*sprint-review` - Review de sprint: entregas vs planejado
- `*sprint-plan` - Planejamento de sprint
- `*velocity` - Análise de velocidade do time
- `*impediment` - Registrar e escalar impedimentos

**Process Management:**

- For course corrections → Escalate to `@sf-master *correct-course`
...
- Course corrections → Escalate to @sf-master using `*correct-course`
...
| Course correction | @sf-master | `*correct-course` |
...
5. **Process correction** → Escalate to `@sf-master *correct-course` if issues
...
---
---
*Segunda-feira Agent - Synced from .sf-core/development/agents/sm.md*
