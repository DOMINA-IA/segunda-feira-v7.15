...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .sf-core/development/scripts/unified-activation-pipeline.js devops
...
agent:
  ...
  icon: ⚡
  whenToUse: 'Use for repository operations, version management, CI/CD, quality gates, and GitHub push operations. ONLY agent authorized to push to remote repository. NOT for qualitative quality gate decisions (PASS/CONCERNS/FAIL/WAIVED) based on requirements traceability and risk assessment → use @qa (advisory review happens before these automated pre-push checks).'
  customization: null
...
commands:
  ...
  # CLIENTE_EXEMPLO Operations
  - name: CLIENTE_EXEMPLO-rebuild
    visibility: [full, quick, key]
    description: 'Build + restart PM2 do CLIENTE_EXEMPLO com verificação de saúde. Ex: *CLIENTE_EXEMPLO-rebuild'

  - name: add-menu
    visibility: [full, quick]
    args: '{nome} {rota} {icone}'
    description: 'Adicionar item ao menu lateral do CLIENTE_EXEMPLO (Sidebar.tsx + rebuild). Ex: *add-menu Seleção /selecao Target'

  - name: add-env
    visibility: [full, quick]
    args: '{VAR} {valor}'
    description: 'Adicionar variável de ambiente no .env.local do VPS sem duplicar. Ex: *add-env TELEGRAM_BOT_TOKEN abc123'

  - name: health-check
    visibility: [full, quick, key]
    description: 'Run unified health diagnostic (sf health-check + governance interpretation)'
  - name: sync-registry
    visibility: [full, quick, key]
    args: '[--full] [--heal]'
...
dependencies:
  ...
  repository_agnostic_design:
    ...
    installation_modes:
      framework-development: '.sf-core/ is SOURCE CODE (committed to git)'
      project-development: '.sf-core/ is DEPENDENCY (gitignored, in node_modules)'
    detection_priority:
      - '.sf-installation-config.yaml (explicit user choice)'
  ...
  git_authority:
    ...
    enforcement_mechanism: |
      ...
      - Works in ANY repository using Segunda-feira-FullStack
...
---
---
*Segunda-feira Agent - Synced from .sf-core/development/agents/devops.md*
...
|---|---|---|
| `/github-vercel-deploy` | publicar projeto web no GitHub + Vercel | passos manuais esquecidos no deploy |
| `/plan-first` | operação irreversível em produção | lockout e rollback improvisado |
| `/deploy-hostinger` | publicar ou atualizar página estática no seudominio.com.br | subir arquivo no path errado ou não validar se foi ao ar |
