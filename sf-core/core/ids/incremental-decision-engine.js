...
class IncrementalDecisionEngine {
  ...
  _generateOverallRationale(recommendations, topDecision, evaluations) {
    if (recommendations.length === 0) {
      return 'No matches found CLIENTE_EXEMPLO minimum threshold. CREATE is recommended.';
    }
    ...
    const parts = [`Found ${evaluations.length} match(es) CLIENTE_EXEMPLO threshold.`];
