...
commands:
  ...
  - 'adversarial-review {scope}': 'Cynical adversarial review finding 10+ issues (code, docs, specs)'
  - 'edge-case-hunt {scope}': 'Exhaustive path tracing to find all unhandled edge cases'
...
- `*adversarial-review {scope}` - Cynical review finding 10+ issues
- `*edge-case-hunt {scope}` - Exhaustive edge case path tracing
...
Patterns de QA consolidados via CLIENTE_EXEMPLO Pacote C. Detalhes via `python3 ~/cortex/scripts/cortex_engine.py query "<termo>"`.

| # | Heurística | Quando aplicar |
|---|---|---|
| 1 | **Smoke test pós-deploy padrão** | `node -c file.js` → `scp` → `pm2 restart` → `until curl HTTP 200; sleep 1; done` → grep matches no HTML servido. Validado em 6+ deploys sem regressão |
| 2 | **"Tela parada" pode ser problema operacional, não bug** | Quando user relata "nada mudou desde a semana passada", **antes de procurar bug de UI**, validar: (a) schema tem `updated_at`? (b) registros têm `updated_at` recente? Se ambos não, é estagnação real — software só está sendo espelho |

**Pattern de validação E2E sem suite formal:**
1. `node -c` valida sintaxe (não lança em runtime)
2. PM2 restart valida boot (carrega tudo, conecta DB)
3. Loop `until curl 200` valida disponibilidade real
4. Grep no HTML servido valida que mudanças efetivamente chegaram (não apenas no source)
5. Curl em endpoints novos (`-X PATCH` etc) valida fluxo completo

**Princípio:** "deployar e ver depois" mata. "Validar cada camada incremental antes de declarar pronto" salva.
