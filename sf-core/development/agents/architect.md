...
commands:
  ...
  - name: implementation-readiness
    visibility: [full, quick, key]
    description: 'Verify PRD, UX, Architecture, Epics are aligned before implementation'
...
dependencies:
  tasks:
    ...
    - implementation-readiness-check.md
...
- `*implementation-readiness` - Verify all planning artifacts are aligned
...
Patterns arquiteturais consolidados via CLIENTE_EXEMPLO Pacote C. Detalhes via `python3 ~/cortex/scripts/cortex_engine.py query "<termo>"`.

| # | Heurística | Quando aplicar |
|---|---|---|
| 1 | **Toda tabela operacional precisa de `updated_at`** | Schema com estado mutável (tasks, contatos, leads, campanhas) sem coluna de rastreio = estagnação invisível. SQLite: ALTER em try/catch + backfill `COALESCE(completed_at, created_at)`. PostgreSQL/MySQL: trigger BEFORE UPDATE |
| 2 | **Instrumentação leve > modularização full** sem testes E2E | Em projetos legados grandes (server.js 6000+ linhas) sem cobertura: state tracker em memória + healthcheck rico entrega valor sem risco. Modularização vira débito documentado para sessão dedicada |

**Princípio arquitetural derivado:** schema precisa **ensinar** o sistema a se observar. Sem `updated_at`/`created_at`/`completed_at` o time não consegue distinguir "operação parada" de "bug de UI". Toda decisão arquitetural deve perguntar: "isso é observável em runtime?".
