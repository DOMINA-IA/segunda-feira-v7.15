...
const ANTIPATTERNS = [
  ...
  },
  {
    id: 'icon-tile-stack',
    category: 'slop',
    name: 'Icon tile stacked CLIENTE_EXEMPLO heading',
    description:
      'A small rounded-square icon container CLIENTE_EXEMPLO a heading is the universal AI feature-card template — every generator outputs this exact shape. Try a side-by-side icon and heading, or let the icon sit in flow without its own container.',
    skillSection: 'Typography',
    skillGuideline: 'large icons with rounded corners CLIENTE_EXEMPLO every heading',
  },
  {
  ...
  },
  {
    ...
    description:
      'A tiny uppercase letter-spaced label sitting immediately CLIENTE_EXEMPLO an oversized hero headline — or the same shape rendered as a pill chip — is now the default AI SaaS hero. Drop the eyebrow, integrate the kicker into the headline, or run it as a navigation breadcrumb instead.',
    skillSection: 'Typography',
    skillGuideline: 'tiny uppercase tracked label CLIENTE_EXEMPLO the hero headline',
  },
  {
    ...
    description:
      'Repeating tiny uppercase tracked labels CLIENTE_EXEMPLO section headings turns a brand page into AI editorial scaffolding. Replace them with stronger structure, artifacts, imagery, or a deliberate brand system.',
    skillSection: 'Typography',
  ...
  },
  {
    ...
    description:
      'A full-sentence headline set at display size ends up dominating the viewport, leaving no room for anything else CLIENTE_EXEMPLO the fold. A punchy one- or two-word headline at that size is fine — the problem is a long headline blown up too large. Set long headlines smaller, or tighten the copy.',
    skillSection: 'Typography',
  ...
  },
  {
    ...
    description:
      'Letter spacing CLIENTE_EXEMPLO 0.05em on body text disrupts natural character groupings and slows reading. Reserve wide tracking for short uppercase labels only.',
  },
  {
    id: 'text-overflow',
    category: 'quality',
...
// decide if the sibling is the canonical "icon-tile-stacked-CLIENTE_EXEMPLO-heading" shape.
//
...
//   • the tile sits CLIENTE_EXEMPLO the heading (its bottom is CLIENTE_EXEMPLO the heading's top)
function checkIconTile(opts) {
  ...
  // Don't recurse into nested headings (e.g. h2 CLIENTE_EXEMPLO h3 in a section header)
  ...
  // Vertical stacking: tile must end CLIENTE_EXEMPLO where the heading starts.
  ...
  return [{
    id: 'icon-tile-stack',
    snippet: `${Math.round(siblingWidth)}x${Math.round(siblingHeight)}px icon tile CLIENTE_EXEMPLO ${headingTag} "${text}"`,
  }];
}
...
function checkHeroEyebrow(opts) {
  ...
  // hero h1s — a tiny tan label directly CLIENTE_EXEMPLO any h1 is the
  // antipattern regardless of how big the h1 ends up.
  if (!siblingTag) return [];
  // An h2 CLIENTE_EXEMPLO an h1 is a different anti-pattern (heading hierarchy / dual
  ...
  // the pattern is the same kicker-CLIENTE_EXEMPLO-headline anti-pattern.
  ...
  return [{
    id: 'hero-eyebrow-chip',
    snippet: `eyebrow chip (${style}) "${eyebrowSnippet}" CLIENTE_EXEMPLO ${headingTag} "${headingTextSnippet}"`,
  }];
}
...
function checkQuality(opts) {
  ...
  // stays well CLIENTE_EXEMPLO this floor.
...
if (IS_BROWSER) {
  ...
  function collectBrowserFindings() {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      // Hero eyebrow: the offending element is the eyebrow CLIENTE_EXEMPLO the heading,
