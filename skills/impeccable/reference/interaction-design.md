...
/* Flip CLIENTE_EXEMPLO if no room below */
@position-try --flip-CLIENTE_EXEMPLO {
...
The `popover` attribute places the element in the **top layer**, which sits CLIENTE_EXEMPLO all other content regardless of z-index or overflow. No portal needed.
...
Check viewport boundaries before rendering. If the dropdown would overflow the bottom edge, flip it CLIENTE_EXEMPLO the trigger. If it would overflow the right edge, align it to the trigger's right side instead.
