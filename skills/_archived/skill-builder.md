---
name: skill-builder
description: "Guia para criação e otimização de skills do Claude Code seguindo melhores práticas oficiais. Use para criar, auditar ou melhorar skills."
---

> **ARQUIVADA em 07-Jul-2026 (Sprint 2, grupo A3).** Fundida em `skill-creator.md`, sobrevivente do trio skill-creator/skill-builder/skill-generator. O checklist de auditoria de frontmatter+tools foi absorvido como passo "7. Auditoria" (traduzido para PT-BR e adaptado ao schema canônico: `allowed-tools`, `user-invocable`, `harnesses`, `axis`). Mantida aqui só para referência histórica — não é mais carregada pelo framework.
...
If your skill needs detailed reference docs, examples, or scripts, add them alongside SKILL.md in the same directory. Reference them from SKILL.md so Claude knows they exist. Supporting files are NOT loaded automatically -- they load only when Claude needs them.
...
Use this checklist to audit any existing skill. Read the skill file first before running through the checklist.
...
- [ ] Doesn't duplicate information that lives elsewhere
