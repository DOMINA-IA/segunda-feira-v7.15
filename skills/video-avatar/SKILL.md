---
name: video-avatar
description: "Cria vídeo com pessoa sintética em 2 modos — clone do ${CEO_NAME} (HeyGen+ElevenLabs, ~R$5/Reel) ou influencer virtual do zero. Use para Reel, VSL ou ad sem gravar. NOT for: movimento de câmera cinematográfico — é /camera-moves."
version: 2.0.0
...
> **Funde `video-avatar` + `criar-video-avatar` (05-Set-2026).** O Modo 1 apenas apontava para a
> outra skill, e os dois arquivos divergiam no custo (R$3 vs R$5). Agora o pipeline vive aqui.
...
## Modo 1 — Clone do ${CEO_NAME} (pipeline validado em produção)

**Arquitetura:** ElevenLabs gera o áudio, HeyGen só faz o lipsync. **Nunca usar o TTS interno do
HeyGen** (`voice.type: text`) — soa robotizado e custa 4× mais.

| Componente | Recurso | Por quê |
|---|---|---|
| Voz | ElevenLabs clone `${PLATFORM_ID}` | natural; TTS do HeyGen é robotizado |
| Avatar | HeyGen Video Avatar `${PLATFORM_ID}` | movimento corporal real (vídeo treinado) |
| Hosting do áudio | litterbox.catbox.moe (24h) | grátis, o HeyGen só precisa baixar |
| Pós-produção | `~/projetos/dominaia/video-pipeline-local/` | legendas word-slam + overlays + quality gate |

**Fluxo:**
1. ElevenLabs gera MP3 com a voz clonada
2. Upload para litterbox.catbox.moe → URL pública de 24h
3. HeyGen `POST /v2/video/generate` com `voice.type: audio` + `audio_url`
4. HeyGen sincroniza apenas a boca
5. Pipeline DOMINA.IA aplica legendas e quality gate **antes** de publicar

IDs e chaves em `~/projetos/dominaia/video-pipeline-local/.env`.

**Custo: ~R$5 por Reel de 60s** — ElevenLabs dentro do plano Creator + ~60 créditos HeyGen.
(A versão anterior desta skill dizia R$3; o número foi recalculado em 05-Set-2026 a partir da
tabela de custo real. Divergência causada por manter o mesmo fato em dois arquivos.)
...
- Knowledge base completa: `~/projetos/dominaia/fabrica-conteudo-invisivel/KNOWLEDGE-BASE-COMPLETA.md`
- Cursos consolidados:
...
- Transcrições completas em `~/projetos/dominaia/fabrica-conteudo-invisivel/transcricoes/` e `~/projetos/dominaia/fabrica-conteudo-invisivel/transcricoes-cakto/`
- Resumos executivos em `~/projetos/dominaia/fabrica-conteudo-invisivel/resumos/`
