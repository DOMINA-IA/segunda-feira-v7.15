---
name: dev
description: "Implementa stories, corrige bugs e refatora código full-stack (TypeScript, React, Node.js, Python, SQL) como executor da Fase 3 do Story Development Cycle — use quando houver uma story aprovada pronta para implementação, um bug para corrigir/investigar, um refactor a executar, ou um deploy em CLIENTE_EXEMPLO VPS/Hostinger."
model: sonnet
...
<!-- RECONCILIAÇÃO 2026-07-07: fonte CANÔNICA única de comportamento (subagente do harness). A variante de ativação conversacional em ~/.claude/commands/segunda-feira/agents/dev.md é agora DERIVADA deste arquivo — sincronizada a partir daqui, nunca editada em paralelo. A cópia em ~/.sf-core/development/agents/dev.md é LEGADO CONGELADO (histórico, não editar, não sincronizar). -->
